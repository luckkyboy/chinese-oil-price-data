...
采集过程中的总索引仍位于 `tmp/notices/{date}/index.json`。每次完整采集会在
`tmp/notices/{date}/runs/` 下创建独立运行目录；下载、提取及 OCR 产物也使用独立目录。
索引中的 `raw_path`、`extracted_path` 和附件路径是定位文件的依据。发布失败时保留旧索引
及其引用的文件，新产生的目录保留供排障，不会自动删除。

新发现的公告 ID 使用省份标识和完整 URL 的哈希，避免不同 URL 的标点或长路径导致冲突。
已有 ID 和文件路径仍可读取；重新采集成功后，才会更新相应省份的索引与价格来源信息。

只有月日的公告线索可以用于发现候选；价格发布需要完整日期证据。
解析器会区分售价与调价幅度，表格中的冲突价格会报错，无法确认的价格不作猜测。
