...
# The HTML body can contain attachment labels before the actual OCR table.
# Allow OCR line breaks inside the title and select the last occurrence below.
GASOLINE_TABLE_RE = re.compile(r"汽油[\s\S]{0,80}?价格表")
DIESEL_TABLE_RE = re.compile(r"柴油[\s\S]{0,80}?价格表")
...
def _table_start(text: str, header_pattern: re.Pattern[str], legacy_marker: str) -> int | None:
    ...
    if matches:
        # html_to_text may contain an early attachment description and the
        # actual table appears later in the OCR text.  The final header is the
        # table that should delimit the price section.
        return matches[-1].start()
    markers = [match.start() for match in re.finditer(re.escape(legacy_marker), text)]
