...
PRODUCT_LINE_PATTERNS = {
    "89": re.compile(r"^\s*89\s*[#﹟号]?\s*.*汽油", re.IGNORECASE),
    "92": re.compile(r"^\s*92\s*[#﹟号]?\s*.*汽油", re.IGNORECASE),
    "95": re.compile(r"^\s*95\s*[#﹟号]?\s*.*汽油", re.IGNORECASE),
    "0": re.compile(r"^\s*0\s*[#﹟号]?\s*.*柴油", re.IGNORECASE),
}
    ...
    for index, raw_line in enumerate(lines):
        ...
        for product, pattern in PRODUCT_LINE_PATTERNS.items():
            if product in prices or not pattern.search(line):
                continue
            # Anhui layout has sparse blank lines; widen the scan window to capture liter price reliably.
